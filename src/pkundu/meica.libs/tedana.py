...
def spatclust(data,mask,csize,thr,header,aff,infile=None,dindex=0,tindex=0):
	...
	return fmask(nib.load('__clout.nii.gz').get_data(),mask)!=0
...
def t2smap(catd,mask,tes):
	...
	return unmask(t2s,mask), unmask(s0,mask), unmask(t2s_fit,mask)
...
def writefeats(cbetas,comptable,mmix,suffix=''):
	...
	if options.e2d is None: e2d=int(np.floor(ne/2))+1
